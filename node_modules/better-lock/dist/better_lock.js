...
var BetterLock = /** @class */ (function () {
     ...
     * @param jobOptions Options to be applied on this job only
     */
    BetterLock.prototype.acquireOr = function (key, failureResult, executor, callback, jobOptions) {
        ...
        if (tools.isFunction(callback)) {
            ...
            callback = function (err) {
                ...
                if ((0, errors_1.isAcquisitionFailureError)(err)) {
                    originalCallback_1(null, failureResult);
                }
                else {
                    originalCallback_1.apply(void 0, __spreadArray([err], results, false));
                }
            };
        }
        // noinspection JSVoidFunctionReturnValueUsed
        var promiseResult = this.acquire(key, executor, callback, jobOptions);
        if (typeof promiseResult !== 'undefined') {
            promiseResult = promiseResult.catch(function (err) {
                if ((0, errors_1.isAcquisitionFailureError)(err)) {
    ...
    BetterLock.prototype.update = function (queues) {
        for (var i = 0; i < queues.length; i++) {
            ...
            if (tools.isNumber(this.options.queue_size) && this.options.queue_size < queue.jobs.length) {
                var strategy = this.options.queue_ejection_strategy || 'oldest';
                var targetJob = queue.jobs[strategy === 'oldest' ? 0 : queue.jobs.length - 1];
                this.log("".concat(queue.toString(), " has overflown, so the ").concat(strategy, " job (").concat(targetJob, ") was kicked out"));
                this.endJob(targetJob, [
                    new errors_1.BetterLockQueueOverflowError(this.options.name, queue.key, queue.jobs.length, targetJob, strategy),
                ]);
            }
        }
    };
    BetterLock.prototype.executeJob = function (job) {
        ...
        setImmediate(function () {
            ...
            for (var i = 0; i < job.keys.length; i++) {
                ...
                if (queue.active !== job) {
                    ...
                    return _this.endJob(job, [
                        new errors_1.BetterLockInternalError(_this.options.name, "Corrupted wait queue state for ".concat(queue.key)),
                    ]);
                }
            }
            ...
            var lockDone = function () {
                ...
                _this.log("Done called for ".concat(job));
    ...
    BetterLock.prototype.onWaitTimeout = function (job) {
        this.log("".concat(job, " has timed out after waiting in queue for ").concat(new Date().valueOf() - job.enqueued_at.valueOf(), "ms"));
    ...
    BetterLock.prototype.onExecutionTimeout = function (job) {
        this.log("".concat(job, " has timed out after executing for ").concat(new Date().valueOf() - job.executed_at.valueOf(), "ms"));
    ...
    BetterLock.prototype.endJob = function (job, callbackArgs) {
        var _this = this;
        if (job.ended_at) {
            this.log("WARNING: ".concat(job, " is trying to end, but it has already ended at ").concat(job.ended_at.toISOString(), ". Called with arguments: ").concat(callbackArgs));
            return;
        }
        clearTimeout(job.wait_timeout_id);
        job.wait_timeout_id = null;
        ...
        for (var i = 0; i < job.keys.length; i++) {
            ...
            if (queue.active === job) {
                ...
                if (index >= 0) {
                    ...
                    this.log("WARNING: ".concat(job, " is ending, but it is not found anywhere in the queue for ").concat(queue.key));
                }
            }
        }
        // Call the callback
